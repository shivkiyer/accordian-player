...
export default function ProgressBar() {
  ...
  const progressMousePositionX = useAppSelector(selectProgressMousePositionX);
  ...
  const calculateVideoPosition = useCallback(
    (mousePosition: number, sideMargin: number): number | null => {
      const { x: xMin, width } = (
        progressRef as any
      ...
      let videoPositon: number | null;
      if (width > 0) {
        videoPositon = videoLocation / (width - 2 * sideMargin);
      } else {
        videoPositon = null;
      }
      if (videoPositon !== null && videoPositon > 1) {
        videoPositon = 1;
      }
      if (videoPositon !== null && videoPositon < 0) {
        videoPositon = 0;
      }
      return videoPositon;
    },
   ...
   */
  const updateVideoTime = (event: React.MouseEvent<HTMLElement>) => {
